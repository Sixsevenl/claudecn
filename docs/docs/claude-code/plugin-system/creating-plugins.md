...
如何创建 Claude Code 插件。本文提供完整的代码示例和终端操作演示。
...
Claude: 如何创建 Claude Code 插件的核心概念包括...
```
...
response = client.messages.create(
    ...
    max_tokens=1024,
    messages=[{"role": "user", "content": "如何创建 Claude Code 插件"}]
)
...
if __name__ == "__main__":
    result = process("如何创建 Claude Code 插件")
    print(result)
```
...
tools = [{
    "name": "process",
    "description": "如何创建 Claude Code 插件",
    "input_schema": {
        "type": "object",
        "properties": {
            "query": {"type": "string"}
...
response = client.messages.create(
    ...
    system=[{
        "type": "text",
        "text": "你是一个如何创建 Claude Code 插件专家...",
