...
通过 Amazon Bedrock 使用 Claude。本文提供完整的代码示例和终端操作演示。
...
You: 帮我实现通过 Amazon Bedrock 使用 Claude
...
```bash
You: 解释一下Amazon Bedrock的核心概念

Claude: 通过 Amazon Bedrock 使用 Claude的核心概念包括...
```
...
response = client.messages.create(
    ...
    max_tokens=1024,
    messages=[{"role": "user", "content": "通过 Amazon Bedrock 使用 Claude"}]
)
...
if __name__ == "__main__":
    result = process("通过 Amazon Bedrock 使用 Claude")
    print(result)
```
...
tools = [{
    "name": "process",
    "description": "通过 Amazon Bedrock 使用 Claude",
    "input_schema": {
        "type": "object",
        "properties": {
            "query": {"type": "string"}
...
response = client.messages.create(
    ...
    system=[{
        "type": "text",
        "text": "你是一个通过 Amazon Bedrock 使用 Claude专家...",
