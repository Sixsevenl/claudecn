...
在 JetBrains IDE 中使用 Claude Code。本文提供完整的代码示例和终端操作演示。
...
You: 帮我实现在 JetBrains IDE 中使用 Claude Code
...
```bash
You: 解释一下JetBrains 集成的核心概念

Claude: 在 JetBrains IDE 中使用 Claude Code的核心概念包括...
```
...
response = client.messages.create(
    ...
    max_tokens=1024,
    messages=[{"role": "user", "content": "在 JetBrains IDE 中使用 Claude Code"}]
)
...
```bash
You: 帮我实现JetBrains 集成的核心功能

Claude: 好的，让我创建实现文件...

Claude wants to:
  • Write file: src/jetbrains_集成.py
Allow? [y/n/always/never]
...
if __name__ == "__main__":
    result = process("在 JetBrains IDE 中使用 Claude Code")
    print(result)
```
...
tools = [{
    "name": "process",
    "description": "在 JetBrains IDE 中使用 Claude Code",
    "input_schema": {
        "type": "object",
        "properties": {
            "query": {"type": "string"}
...
response = client.messages.create(
    ...
    system=[{
        "type": "text",
        "text": "你是一个在 JetBrains IDE 中使用 Claude Code专家...",
