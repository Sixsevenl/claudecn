...
Agent Skills 的架构设计原理。本文提供完整的代码示例和终端操作演示。
...
```bash
You: 解释一下Skills 架构设计的核心概念

Claude: Agent Skills 的架构设计原理的核心概念包括...
```
...
response = client.messages.create(
    ...
    max_tokens=1024,
    messages=[{"role": "user", "content": "Agent Skills 的架构设计原理"}]
)
...
```bash
You: 帮我实现Skills 架构设计的核心功能

Claude: 好的，让我创建实现文件...

Claude wants to:
  • Write file: src/skills_架构设计.py
Allow? [y/n/always/never]
...
if __name__ == "__main__":
    result = process("Agent Skills 的架构设计原理")
    print(result)
```
...
tools = [{
    "name": "process",
    "description": "Agent Skills 的架构设计原理",
    "input_schema": {
        "type": "object",
        "properties": {
            "query": {"type": "string"}
...
response = client.messages.create(
    ...
    system=[{
        "type": "text",
        "text": "你是一个Agent Skills 的架构设计原理专家...",
